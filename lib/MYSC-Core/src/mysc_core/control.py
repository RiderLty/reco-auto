...
import struct
import threading
from typing import ClassVar, Callable, Optional, Self
...
class Gamepad:
    ...
    class DPad:
        ...
        dpad_keys: ClassVar[list[UnifiedKey]] = [
...
@dataclass
class ControlKwargs(ConnectKwargs):
    """
        控制连接参数
    """
    class EnumScreenStatus(StrEnum):
        OFF = 'off'
        ON = 'on'
        KEEP = 'keep'
    ...
    show_touches: Optional[bool] = None
    display_id: Optional[int] = None

    _clipboard: bool = True
    _screen_status: EnumScreenStatus = EnumScreenStatus.KEEP
...
class ControlAdapter(Adapter):
    ...
    @staticmethod
    def get_window_size(adb_device: AdbDevice, display_id: Optional[int] = None) -> Coordinate:
        """
        ...
        :return:
        """
        cmd = f"wm size -d {display_id}" if display_id else "wm size"
    ...
    def __init__(
            self,
            control_kwargs: Optional[ControlKwargs] = ControlKwargs(),
        ...
        """
        super().__init__(control_kwargs)
        self.screen_status: ControlKwargs.EnumScreenStatus = self._kwargs._screen_status
    ...
    def connect(self, adb_device: AdbDevice, *args, **kwargs) -> Self:
        ...
        :return:
        """
        logger.debug(f"| > Control Args | {self.connect_kwargs}")

        self.set_wm_coord(self.get_window_size(adb_device, self._kwargs.display_id))

        if self.screen_status == ControlKwargs.EnumScreenStatus.KEEP:
        ...
        threading.Thread(target=self.thread_clipboard, daemon=True).start()

        self.f_set_screen(
            True if self.screen_status == ControlKwargs.EnumScreenStatus.ON else False
        )

        return self

    def set_wm_coord(self, wm_coord: Coordinate):
        """
            重新设置窗口坐标
            Crop时使用
        :param wm_coord:
        ...
        wm_coord = wm_coord.fit_scrcpy_video()

        if wm_coord.current_direction == EnumDirection.HORIZONTAL:
            self.coord_hv[EnumDirection.HORIZONTAL] = wm_coord
            self.coord_hv[EnumDirection.VERTICAL] = wm_coord.rotate()
        else:
            self.coord_hv[EnumDirection.VERTICAL] = wm_coord
            self.coord_hv[EnumDirection.HORIZONTAL] = wm_coord.rotate()
    ...
    def thread_main(self):
        ...
        while self.is_running:
            try:
                ...
                logger.warning(f"| ! Exception while sending control! > {e}")

        logger.info(f"| - Control closed.")
        ...
        while self.is_running:
            try:
                ...
                if _bs == b'':
                    ...
                    if self._kwargs._clipboard:
                        pyperclip.copy(_bs[5:5+_size].decode('utf-8'))
            except OSError:
                pass
            except Exception as e:
                logger.warning(f"| ! Exception while read clipboard! > {e}")

        logger.info(f"| - Clipboard closed.")

    # ---------------------------- Functions ----------------------------

    def send_packet(self, packet: bytes, ignore_repeat_check: bool = False):
        """
            发送控制数据包
        :param packet:
        :param ignore_repeat_check: 忽略重复命令检查
        :return:
        """
        if packet != self.last_packet or ignore_repeat_check:
    ...
    def packet__touch(
        ...
        return struct.pack(
            ...
            *[
                ...
                0 if action == EnumAction.UP else 0xffff,               # 6 H pressure
    ...
    def f_touch(
            ...
            width: int, height: int,
            touch_id: int,
            ignore_repeat_check: bool = False
    ):
        self.send_packet(
            self.packet__touch(
    ...
    def f_touch_spr(
        ...
        :param scale_point_r:
        :param touch_id:
        :param ignore_repeat_check:
        :return:
        """
        _coord = self.coord_hv[scale_point_r.r]
        x, y = _coord.to_point(scale_point_r).t
        self.f_touch(
            action,
            x + (self.offset_x if scale_point_r.r == EnumDirection.VERTICAL else self.offset_y),
            y + (self.offset_y if scale_point_r.r == EnumDirection.VERTICAL else self.offset_x),
            **_coord.d,
            touch_id=touch_id,
            ignore_repeat_check=ignore_repeat_check
        )
    ...
    @classmethod
    def packet__scroll_event(
            cls,
            x: int, y: int,
            width: int, height: int,
            h_scroll: int, v_scroll: int, buttons: int = 1
    ):
        """
            Scroll Event
        :param x:
        ...
        return struct.pack(
            '>BiiHHhhI',
            *[
                cls.MessageType.INJECT_SCROLL_EVENT,
                x, y,
                width, height,
                h_scroll, v_scroll,
                buttons
    ...
    def f_scroll(
            self, scale_point_r: ScalePointR, h_scroll: int = 0, v_scroll: int = 0, buttons: int = 1
    ):
        """
            Scroll
        :param scale_point_r:
        ...
        """
        _coord = self.coord_hv[scale_point_r.r]
        self.send_packet(self.packet__scroll_event(
...
if __name__ == '__main__':
    ...
    device = adb.device_list()[1]
    ca = ControlAdapter(ControlKwargs(_screen_status=ControlKwargs.EnumScreenStatus.ON)).connect(device)

    ca.f_touch_spr(
        EnumAction.DOWN, ScalePointR(0.5, 0.3, EnumDirection.HORIZONTAL),0
    )
    time.sleep(1)
    ca.f_touch_spr(
        EnumAction.UP, ScalePointR(0.5, 0.3, EnumDirection.HORIZONTAL),0
    )
